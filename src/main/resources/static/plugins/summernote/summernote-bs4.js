	...
	return /******/ (() => { // webpackBootstrap
		...
		(() => {
			...
			function isInline(node) {
				return !isBodyContainer(node) && !isList(node) && !isHr(node) && !isPara(node) && !isTable(node) && !isBlockquote(node) && !isData(
						node);
			}
				...
				_createClass(Context, [{
					...
					value : function _initialize() {
						...
						var modules = external_jQuery_default().extend({},
						                                               ...
						                                               (external_jQuery_default()).summernote.plugins || {}); // add and initialize modules
			...
			external_jQuery_default().fn.extend({
				                                    ...
				                                    summernote : function summernote() {
					                                    ...
					                                    options.langInfo = external_jQuery_default().extend(true,
					                                                                                        {},
					                                                                                        (external_jQuery_default()).summernote.lang["en-US"],
					                                                                                        (external_jQuery_default()).summernote.lang[options.lang]);
					                                    options.icons = external_jQuery_default().extend(true,
					                                                                                     {},
					...
					value : function normalize() {
						...
						var getVisiblePoint = function getVisiblePoint(point, isLeftToRight) {
							...
							if (dom.isVisiblePoint(point)) {
								if (!dom.isEdgePoint(point) || dom.isRightEdgePoint(point) && !isLeftToRight || dom.isLeftEdgePoint(point) && isLeftToRight || dom.isRightEdgePoint(
										point) && isLeftToRight && dom.isVoid(point.node.nextSibling) || dom.isLeftEdgePoint(point) && !isLeftToRight && dom.isVoid(
										point.node.previousSibling) || dom.isBlock(point.node) && dom.isEmpty(point.node)) {
									return point;
								}
			...
			const range = ({
				...
				createFromSelection : function createFromSelection() {
					...
					if (env.isW3CRangeSupport) {
						...
						if (dom.isText(startPoint.node) && dom.isLeftEdgePoint(startPoint) && dom.isTextNode(endPoint.node) && dom.isRightEdgePoint(
								endPoint) && endPoint.node.nextSibling === startPoint.node) {
							startPoint = endPoint;
						}
					...
					value : function releaseList(clustereds, isEscapseToBody) {
						...
						external_jQuery_default().each(clustereds, function (idx, paras) {
							...
							if (headList.parentNode.nodeName === 'LI') {
								...
								paras = isEscapseToBody ? dom.listDescendant(middleList, dom.isLi) : lists.from(middleList.childNodes)
					...
					value : function addRow(rng, position) {
							...
							switch (currentCell.action) {
								...
								case TableResultAction.resultAction.SumSpanCount: {
									if (position === 'top') {
										...
										var isTopFromRowSpan = (!baseCellTr
										                        ? 0
										                        : currentCell.baseCell.closest('tr').rowIndex) <= currentTr[0].rowIndex;

										if (isTopFromRowSpan) {
											var newTd = external_jQuery_default()('<div></div>')
													.append(external_jQuery_default()('<td' + tdAttributes + '>' + dom.blank + '</td>').removeAttr(
															'rowspan'))
													.html();
											html.append(newTd);
											break;
						...
						if (position === 'top') {
							...
							if (cellHasRowspan) {
								...
								external_jQuery_default()(external_jQuery_default()(currentTr).parent().find('tr')[lastTrIndex]).after(
										external_jQuery_default()(html));
								return;
							}
				...
				Editor_createClass(Editor, [{
					...
					value : function isLimited(pad, event) {
						...
						if (typeof event !== 'undefined') {
							if (key.isMove(event.keyCode) || key.isNavigation(event.keyCode) || event.ctrlKey || event.metaKey || lists.contains([key.code.BACKSPACE,
							                                                                                                                      key.code.DELETE],
							                                                                                                                     event.keyCode)) {
								return false;
							}
			...
			var Dropzone = /*#__PURE__*/function () {
				function Dropzone(context) {
					...
					this.$dropzone = external_jQuery_default()(['<div class="note-dropzone">',
				...
				Dropzone_createClass(Dropzone, [{
					...
					value : function attachDragAndDropEvent() {
						...
						this.$eventListener.on('dragenter', this.documentEventHandlers.onDragenter).on('dragleave',
						                                                                               this.documentEventHandlers.onDragleave).on(
								'drop',
								...
								value = value.replace(/(<iframe.*?>.*?(?:<\/iframe>)?)/gi, function (tag) {
									...
									try {
										for (_iterator.s() ; !(_step = _iterator.n()).done ;) {
											...
											if (new RegExp('src="(https?:)?\/\/' + src.replace(/[-\/\\^$*+?.()|[\]{}]/g,
											                                                   '\\$&') + '\/(.+)"').test(tag)) {
												return tag;
											}
										}
			...
			var Handle = /*#__PURE__*/function () {
				...
				Handle_createClass(Handle, [{
					...
					value : function initialize() {
						...
						this.$handle = external_jQuery_default()(['<div class="note-handle">',
						                                          ...
						                                          '</div>',
						                                          '</div>'].join('')).prependTo(this.$editingArea);
				...
				AutoLink_createClass(AutoLink, [{
					...
					value : function replace() {
						...
						if (match && (match[1] || match[2])) {
							...
							var urlText = this.options.showDomainOnlyForAutolink ? keyword.replace(
			...
			function AutoReplace_createClass(Constructor,
			                                 protoProps,
			                                 staticProps) {
				if (protoProps) {
			...
			function Placeholder_createClass(Constructor,
			                                 protoProps,
			                                 staticProps) {
				if (protoProps) {
									                                                                   ...
									                                                                   toggle : 'dropdown'
								                                                                                        ...
								                                                                                        callback : function callback($dropdown) {
									                                                                                        $dropdown.find(
											                                                                                        '.note-holder')
									                                                                                                 .each(function (idx,
									                                                                                                                 item) {
										                                                                                                 var $holder = external_jQuery_default()(
												                                                                                                 item);
										                                                                                                 $holder.append(
												                                                                                                 _this.ui.palette(
														                                                                                                 {
								                                                                                        ...
								                                                                                        click : function click(event) {
									                                                                                        ...
									                                                                                        var $button = external_jQuery_default()(
											                                                                                        event.target);
									                                                                                        var eventName = $button.data(
											                                                                                        'event');
									                                                                                        var value = $button.attr(
											                                                                                        'data-value');
									                                                                                        ...
									                                                                                        if (eventName === 'openPalette') {
										                                                                                        ...
										                                                                                        var $palette = external_jQuery_default()(
												                                                                                        $parent.find(
														                                                                                        '#' + $picker.data(
																                                                                                            'event'))
												                                                                                               .find('.note-color-row')[0]); // Shift palette chips
										                                                                                        ...
										                                                                                        if (lists.contains(['backColor',
										                                                                                                            'foreColor'],
										                                                                                                           eventName)) {
											                                                                                        var key = eventName === 'backColor'
											                                                                                                  ? 'background-color'
											                                                                                                  : 'color';
											                                                                                        var $color = $button.closest(
													                                                                                        '.note-color')
											                                                                                                            .find('.note-recent-color');
											                                                                                        var $currentButton = $button.closest(
													                                                                                        '.note-color')
							...
							return _this2.ui.buttonGroup([_this2.button({
								                                            ...
								                                            data : {
									                                            toggle : 'dropdown'
								                                                                        ...
								                                                                        items : _this2.options.fontNames.filter(_this2.isFontInstalled.bind(
										                                                                        _this2)),
								                                                                        title : _this2.lang.font.name,
								                                                                        template : function template(item) {
									                                                                        return '<span style="font-family: ' + env.validFontName(
											                                                                        item) + '">' + item + '</span>';
								                                                                        },
							...
							return _this2.ui.buttonGroup([_this2.button({
								                             ...
								                             callback : function callback($node) {
									                             ...
									                             $catcher.css({
									                                          ...
									                                          }).mouseup(_this2.context.createInvokeHandler('editor.insertTable')).on(
											                             'mousemove',
				...
				LinkDialog_createClass(LinkDialog, [{
					...
					value : function initialize() {
						...
						var body = ['<div class="form-group note-form-group">',
						            "<label for=\"note-dialog-link-txt-".concat(this.options.id, "\" class=\"note-form-label\">")
						            ...
						            "<label for=\"note-dialog-link-url-".concat(this.options.id, "\" class=\"note-form-label\">")
						            ...
						            '</div>',
						            !this.options.disableLinkTarget ? external_jQuery_default()('<div></div>')
								            ...
								            .html() : '',
						            external_jQuery_default()('<div></div>').append(this.ui.checkbox({
							                                                                             className : 'sn-checkbox-use-protocol',
							                                                                             text : this.lang.link.useProtocol,
							                                                                             checked : true
						                                                                             }).render()).html()].join('');
						var buttonClass = 'btn btn-primary note-btn note-btn-primary note-link-btn';
						var footer = "<input type=\"button\" href=\"#\" class=\"".concat(buttonClass, "\" value=\"").concat(this.lang.link.insert,
						                                                                                                    "\" disabled>");
			...
			function LinkPopover_createClass(Constructor,
			                                 protoProps,
			                                 staticProps) {
				if (protoProps) {
			...
			function ImageDialog_createClass(Constructor,
			                                 protoProps,
			                                 staticProps) {
				if (protoProps) {
			...
			var ImageDialog = /*#__PURE__*/function () {
				...
				ImageDialog_createClass(ImageDialog, [{
					...
					value : function initialize() {
						...
						var footer = "<input type=\"button\" href=\"#\" class=\"".concat(buttonClass, "\" value=\"").concat(this.lang.image.insert,
						                                                                                                    "\" disabled>");
						this.$dialog = this.ui.dialog({
			...
			function ImagePopover_createClass(Constructor,
			                                  protoProps,
			                                  staticProps) {
				if (protoProps) {
			...
			function TablePopover_createClass(Constructor,
			                                  protoProps,
			                                  staticProps) {
				if (protoProps) {
			...
			function VideoDialog_createClass(Constructor,
			                                 protoProps,
			                                 staticProps) {
				if (protoProps) {
				...
				VideoDialog_createClass(VideoDialog, [{
					...
					value : function initialize() {
						...
						var body = ['<div class="form-group note-form-group row-fluid">',
						            "<label for=\"note-dialog-video-url-".concat(this.options.id, "\" class=\"note-form-label\">")
						...
						var footer = "<input type=\"button\" href=\"#\" class=\"".concat(buttonClass, "\" value=\"").concat(this.lang.video.insert,
						                                                                                                    "\" disabled>");
						this.$dialog = this.ui.dialog({
					...
					value : function createVideoNode(url) {
						...
						if (ytMatch && ytMatch[1].length === 11) {
							...
							$video = external_jQuery_default()('<iframe>').attr('frameborder', 0).attr('src', vMatch[0] + '/embed/simple').attr(
									'width',
									'600').attr('height', '600').attr('class', 'vine-embed');
							...
							$video = external_jQuery_default()('<iframe allowfullscreen sandbox="allow-same-origin allow-scripts allow-popups">')
									.attr('frameborder', 0)
									.attr('src',
									      '//' + peerTubeMatch[1] + '/videos/embed/' + peerTubeMatch[2] + "?loop=" + loop + "&autoplay=" + autoplay + "&muted=" + muted + (begin > 0
									                                                                                                                                       ? '&start=' + begin
									                                                                                                                                       : '') + (end > 0
									                                                                                                                                                ? '&end=' + start
									                                                                                                                                                : ''))
					...
					value : function createShortcutList() {
						...
						return Object.keys(keyMap).map(function (key) {
							...
							$row.append(external_jQuery_default()('<label><kbd>' + key + '</kdb></label>').css({
									...
									'<span></span>').html(_this.context.memo('help.' + command) || command));
							return $row.html();
						}).join('');
					}
					/**
			...
			function HintPopover_createClass(Constructor,
			                                 protoProps,
			                                 staticProps) {
				if (protoProps) {
			...
			(external_jQuery_default()).summernote = external_jQuery_default().extend((external_jQuery_default()).summernote, {
				...
				options : {
					...
					colorsName : [['Black', 'Tundora', 'Dove Gray', 'Star Dust', 'Pale Slate', 'Gallery', 'Alabaster', 'White'],
			...
			function _typeof(obj) {
				...
				if (typeof Symbol === "function" && typeof Symbol.iterator === "symbol") { _typeof = function _typeof(obj) { return typeof obj; }; } else {
			...
			function renderer_createClass(Constructor, protoProps, staticProps) {
				if (protoProps) {
					renderer_defineProperties(Constructor.prototype,
					                          protoProps);
				}
				if (staticProps) {
			...
			function summernote_bs4_typeof(obj) {
				"@babel/helpers - typeof";
				if (typeof Symbol === "function" && typeof Symbol.iterator === "symbol") { summernote_bs4_typeof = function _typeof(obj) { return typeof obj; }; } else {
			...
			var statusbar = renderer.create(['<output class="note-status-output" role="status" aria-live="polite"></output>',
			...
			var airEditable = renderer.create(['<div class="note-editable" contentEditable="true" role="textbox" aria-multiline="true"></div>',
			...
			var dropdownCheck = renderer.create('<div class="note-dropdown-menu dropdown-menu note-check" role="list"></div>',
			                                    function ($node, options) {
				                                    var markup = Array.isArray(options.items) ? options.items.map(function (item) {
					                                    var value = typeof item === 'string' ? item : item.value || '';
					                                    var content = options.template ? options.template(item) : item;
					                                    return '<a class="dropdown-item" href="#" data-value="' + value + '" role="listitem" aria-label="' + item + '">' + icon(
							                                    options.checkClassName) + ' ' + content + '</a>';
				                                    }).join('') : options.items;
				                                    $node.html(markup).attr({
					                                                            'aria-label' : options.title
				                                                            });
			...
			var dialog = renderer.create('<div class="modal note-modal" aria-hidden="false" tabindex="-1" role="dialog"></div>',
			                             function ($node, options) {
				                             if (options.fade) {
					                             $node.addClass('fade');
				                             }
				                             ...
				                             $node.html(['<div class="modal-dialog">',
				                                         ...
				                                         options.title
				                                         ? '<div class="modal-header">' + '<h4 class="modal-title">' + options.title + '</h4>' + '<button type="button" class="close" data-dismiss="modal" aria-label="Close" aria-hidden="true">&times;</button>' + '</div>'
				                                         : '',
			...
			var popover = renderer.create(['<div class="note-popover popover in">',
			                               '<div class="arrow"></div>',
			...
			var summernote_bs4_checkbox = renderer.create('<div class="form-check"></div>', function ($node, options) {
				$node.html(['<label class="form-check-label"' + (options.id ? ' for="note-' + options.id + '"' : '') + '>',
			...
			var ui = function ui(editorOptions) {
				return {
						...
						return renderer.create('<div class="note-color-palette"></div>', function ($node, options) {
							...
							for (var row = 0, rowSize = options.colors.length ; row < rowSize ; row++) {
								...
								for (var col = 0, colSize = colors.length ; col < colSize ; col++) {
									...
									buttons.push(['<button type="button" class="note-color-btn"',
					...
					button : function button($node, options) {
						return renderer.create('<button type="button" class="note-btn btn btn-light btn-sm" tabindex="-1"></button>',
						                       function ($node, options) {
					...
					createLayout : function createLayout($note) {
						var $editor = (editorOptions.airMode
						               ...
						               : editorOptions.toolbarPosition === 'bottom' ? editor([editingArea([codable(), editable()]),
						                                                                      toolbar(),
						                                                                      statusbar()]) : editor([toolbar(),
						                                                                                              editingArea([codable(),
						                                                                                                           editable()]),
						                                                                                              statusbar()])).render();
