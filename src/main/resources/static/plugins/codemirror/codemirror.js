...
(function (global, factory) {
	...
	function elt(tag, content, className, style) {
		...
		if (typeof content == "string") { e.appendChild(document.createTextNode(content)); } else if (content) {
	...
	var bidiOrdering = (function () {
		...
		function charType(code) {
			if (code <= 0xf7) { return lowTypes.charAt(code) } else if (0x590 <= code && code <= 0x5f4) { return "R" } else if (0x600 <= code && code <= 0x6f9) {
				return arabicTypes.charAt(code - 0x600)
			} else if (0x6ee <= code && code <= 0x8ac) { return "r" } else if (0x2000 <= code && code <= 0x200b) { return "w" } else if (code == 0x200c) { return "b" } else { return "L" }
		}
		...
		return function (str, direction) {
			...
			for (var i$2 = 0, cur = outerType ; i$2 < len ; ++i$2) {
				...
				if (type$1 == "1" && cur == "r") { types[i$2] = "n"; } else if (isStrong.test(type$1)) {
					cur = type$1;
	...
	function extractLineClasses(type, output) {
		if (type) {
			for (; ;) {
				...
				if (output[prop] == null) { output[prop] = lineClass[2]; } else if (!(new RegExp("(?:^|\\s)" + lineClass[2] + "(?:$|\\s)")).test(
		...
		if (sps) {
			for (var i = 0 ; i < sps.length ; ++i) {
				...
				if (fromCmp <= 0 && (sp.marker.inclusiveRight && marker.inclusiveLeft ? cmp(found.to, from) >= 0 : cmp(found.to, from) > 0) ||
				    fromCmp >= 0 && (sp.marker.inclusiveRight && marker.inclusiveLeft ? cmp(found.from, to) <= 0 : cmp(found.from,
				                                                                                                       to) < 0)) { return true }
			}
		}
	}
	...
	function buildLineContent(cm, lineView) {
		...
		for (var i = 0 ; i <= (lineView.rest ? lineView.rest.length : 0) ; i++) {
			...
			if (hasBadBidiRects(cm.display.measure) && (order = getOrder(line,
			                                                             cm.doc.direction))) {
				builder.addToken = buildTokenBadBidi(builder.addToken,
				                                     order);
			}
			builder.map = [];
		...
		for (var j = 0 ; j < lineView.changes.length ; j++) {
			...
			if (type == "text") { updateLineText(cm, lineView); } else if (type == "gutter") {
				updateLineGutter(cm,
				                 ...
				                 lineN,
				                 dims);
			} else if (type == "class") { updateLineClasses(cm, lineView); } else if (type == "widget") { updateLineWidgets(cm, lineView, dims); }
		}
	...
	function updateLineGutter(cm, lineView, lineN, dims) {
		...
		if (cm.options.lineNumbers || markers) {
			...
			var gutterWrap = lineView.gutter = elt("div",
			                                       null,
	...
	function measureCharInner(cm, prepared, ch, bias) {
		...
		if (node.nodeType == 3) { // If it is a text node, use a range to retrieve the coordinates.
			for (var i$1 = 0 ; i$1 < 4 ; i$1++) { // Retry a maximum of 4 times when nonsense rectangles are returned
				...
				if (ie && ie_version < 9 && start == 0 && end == place.coverEnd - place.coverStart) { rect = node.parentNode.getBoundingClientRect(); } else {
	...
	function maybeScrollWindow(cm, rect) {
		...
		if (doScroll != null && !phantom) {
			var scrollNode = elt("div",
			                     "\u200b",
			                     null,
			                     ("position: absolute;\n                         top: " + (rect.top - display.viewOffset - paddingTop(cm.display)) + "px;\n                         height: " + (rect.bottom - rect.top + scrollGap(
					                     cm) + display.barHeight) + "px;\n                         left: " + (rect.left) + "px; width: " + (Math.max(2,
		...
		for (var first = true ; ; first = false) {
			if (!first || !cm.options.lineWrapping || update.oldDisplayWidth == displayWidth(cm)) {
				...
				if (viewport && viewport.top != null) {
					viewport = {
						top : Math.min(cm.doc.height + paddingVert(cm.display) - displayHeight(cm),
						               viewport.top)
					};
				}
		...
		if (opId == hist.lastSelOp ||
		    (origin && hist.lastSelOrigin == origin &&
		     (hist.lastModTime == hist.lastSelTime && hist.lastOrigin == origin ||
		      selectionEventCanBeMerged(doc, origin, lst(hist.done), sel)))) { hist.done[hist.done.length - 1] = sel; } else {
			pushSelectionToHistory(sel,
			                       hist.done);
		}
	...
	function makeChangeSingleDocInEditor(cm, change, spans) {
		...
		if (change.full) { regChange(cm); } else if (from.line == to.line && change.text.length == 1 && !isWholeLineUpdate(cm.doc,
		                                                                                                                   change)) {
			regLineChange(cm,
			              from.line,
			              "text");
	...
	function addLineWidget(doc, handle, node, options) {
		...
		changeLine(doc, handle, "widget", function (line) {
			...
			if (widget.insertAt == null) { widgets.push(widget); } else {
				widgets.splice(Math.min(widgets.length, Math.max(0, widget.insertAt)),
				               0,
				               widget);
			}
			widget.line = line;
	...
	TextMarker.prototype.attachLine = function (line) {
		if (!this.lines.length && this.doc.cm) {
			...
			if (!op.maybeHiddenMarkers || indexOf(op.maybeHiddenMarkers,
			                                      this) == -1) { (op.maybeUnhiddenMarkers || (op.maybeUnhiddenMarkers = [])).push(this); }
		}
	...
	function normalizeKeyName(name) {
		...
		for (var i = 0 ; i < parts.length - 1 ; i++) {
			var mod = parts[i];
			if (/^(cmd|meta|m)$/i.test(mod)) { cmd = true; } else if (/^a(lt)?$/i.test(mod)) { alt = true; } else if (/^(c|ctrl|control)$/i.test(mod)) { ctrl = true; } else if (/^s(hift)?$/i.test(
					mod)) { shift = true; } else { throw new Error("Unrecognized modifier name: " + mod) }
		}
		...
		if (visually) {
			...
			if (order) {
				...
				if (part.level > 0 || cm.doc.direction == "rtl") {
					...
					ch = findFirst(function (ch) { return measureCharPrepared(cm, prep, ch).top == targetTop; },
	...
	function leftButtonDown(cm, pos, repeat, event) {
		...
		if (cm.options.dragDrop && dragAndDrop && !cm.isReadOnly() &&
		    ...
		    (cmp(contained.to(), pos) > 0 || pos.xRel < 0)) { leftButtonStartDrag(cm, event, pos, behavior); } else {
			leftButtonSelect(cm,
			                 event,
			                 pos,
			                 behavior);
	...
	function addEditorMethods(CodeMirror) {
		...
		CodeMirror.prototype = {
			...
			getTokenTypeAt : function (pos) {
				...
				if (ch == 0) { type = styles[2]; } else {
					for (; ;) {
						...
						if ((mid
						     ? styles[mid * 2 - 1]
						     : 0) >= ch) { after = mid; } else if (styles[mid * 2 + 1] < ch) { before = mid + 1; } else {
							type = styles[mid * 2 + 2];
							break
						}
					}
			...
			cursorCoords : function (start, mode) {
				...
				if (start == null) { pos = range.head; } else if (typeof start == "object") { pos = clipPos(this.doc, start); } else {
					pos = start
					      ? range.from()
					      : range.to();
				}
	...
	ContentEditableInput.prototype.receivedFocus = function () {
		...
		var input = this;
		if (this.selectionInEditor()) { setTimeout(function () { return this$1.pollSelection(); }, 20); } else {
			runInOp(this.cm,
